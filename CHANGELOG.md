...
- **The chat budget moved from 220 to 269 words.** The default render measures 265: the four-item guidance default adds 14 words over the old three, and the vocabulary clause below adds the rest. Nothing outside the test suite reads the table, so a budget below its own default fails a test rather than an install.
