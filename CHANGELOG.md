...
- **The instructions named opaque jargon as banned and gave no way to spot it.** A word list holds what someone thought to add; it can never hold a word the model invents mid-sentence, and chat never reaches a gate in any case. The category is replaced by a rule with a test and both sides of the line: prefer the word your reader already uses, never invent one, common domain vocabulary such as `race condition` and `idempotent` is fine, and anything you cannot source goes in plain English. A term that must be used anyway is glossed on first use. The per-turn reminder now says `unfamiliar word` beside link, SHA and line number, moving from 49 words to 51. The chat block grew from 236 words to 263 and its budget from 240 to 267.
