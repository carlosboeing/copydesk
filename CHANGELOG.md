...
- **A pre-commit gate for repositories that only use git.** `copydesk install --git-hook` writes a `pre-commit` hook that runs the new `copydesk check --staged`, which judges staged Markdown against HEAD line by line instead of whole files. A commit touching a clean paragraph passes in a file full of pre-existing errors, an added banned word refuses the commit, and a pure deletion never blocks; the working tree is never judged, because it is not what the commit carries. The decision reuses the write gate's origin attribution - `_attribute_origins`, extracted unchanged from `_compute_edit_origins` so the existing scope tests cover both callers - with git's own `--unified=0` hunks standing in as the edit footprint once a head/index pair passes the character matcher's 2,000-line cap. The four whole-document rules (`long-sentence-rate`, `avg-sentence-length`, `sentence-variation`, `list-dominated`) have no hunk to belong to and block only when they newly fire against HEAD, as at write time. Warn-severity findings print and never block, and `git commit --no-verify` skips the check: that instruction sits inside the installed hook itself, where a reader meets it before they need it. A hook CopyDesk did not write is never touched; the installer prints the lines to append instead. A missing `copydesk` binary or an internal error lets the commit through, keeping a broken install from bricking commits. This widens the command-line surface: `install` leaves the reserved list with `--git-hook` as its first flag, and `check --staged` joins `check --commit-msg`; exit codes are unchanged (0 pass or warn, 1 refused).
