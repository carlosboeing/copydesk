...
- **The write-time gate now blocks on OpenCode.** OpenCode has no shell hooks; its extension point is a plugin module. `hooks/opencode/copydesk-gate.js` maps `tool.execute.before` onto the same `lib/linter.py` — lowercase tool names, camelCase arguments (`filePath`, `oldString`, `newString`) and an injected `replace_all` translate to the shared envelope, and the session id gains an `opencode-` prefix for isolated retry state. OpenCode evaluates the hook twice per call and only the second pass carries a `callID`; the adapter gates on that and caches the verdict per call, so one user-visible write consumes exactly one three-strike attempt instead of two. Throwing from the handler denies the write and shows the findings to the model; any internal failure lets the write through. Setup installs one plugin copy plus the linter bundle under the OpenCode config directory, and uninstall removes them. Verified live against OpenCode 1.18.21: block, block, then escape on the third attempt with the file landing verbatim, and a `.txt` control passing first try.
