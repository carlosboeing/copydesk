...
- **Skipping `~/.grok/AGENTS.md` when Claude compat already delivers the same block**: Grok Build reads `~/.claude/CLAUDE.md` when `[compat.claude] agents = true` in `~/.grok/config.toml`, so a user with both harnesses receives the block twice. Dropping `instruction_file` from the grok adapter fixes that and breaks the Grok-only user, who never gets `~/.claude/CLAUDE.md` written at all: `wizard.py:632` builds targets from `selected_tools`. A rule conditional on both the selected tools and the compat setting would be safe, and it buys 627 words for a narrow population while adding a new way to install nothing. `copydesk doctor` reporting the duplication is the version worth building first, because it decides nothing for the user.
