...
# The block is spliced into a host document with no line naming what it is,
# so its rules read as part of whatever section sits above the markers. The
# heading is fixed at H2: the teaching block already ships `##` headings, and
# a host needing another level is a config knob when the need is felt, not
# before. The notice follows the output style's precedent: invisible in
# rendered prose, present for the agent or editor about to hand-edit.
_BLOCK_HEADING = "## Writing rules"

_BLOCK_NOTICE = (
    "<!-- Generated by CopyDesk. `copydesk setup --repair` rewrites this "
    "region; edit the preset, never the block. -->"
)
...
def render_agents_block(resolved: dict, include_chat: bool = False) -> str:
    ...
    A non-empty block opens with the `_BLOCK_HEADING` and `_BLOCK_NOTICE`
    pair. An empty one stays empty: with every channel off there is nothing
    to head, and a lone heading would splice into the host as a section with
    no body.
    ...
    if not body:
        return ""
    return "\n\n".join([_BLOCK_HEADING, _BLOCK_NOTICE, body])
